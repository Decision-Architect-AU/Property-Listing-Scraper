...
# Sanity cap: residential weekly rent from suburb median.
# Higher values indicate corrupt data (e.g. sale price in rent column).
...
def estimate_rent(desc, beds='', ptype='', suburb='', state=''):
    ...
    Returns: {annual_rent, weekly_rent, assumptions, rent_source}
    """
    ...
    # 2. Individual units: "Unit 1: $350pw, Unit 2: $380pw"
    ...
    # 5. Suburb median rent fallback
    # Guard against corrupt stats rows where sale price lands in the rent column
    # (e.g. Clinton SA has rent_house_pw = 297500 — clearly a mis-mapped price).
    if suburb:
