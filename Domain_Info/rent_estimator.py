...
Handles:
  ...
  - Multi-unit desc: "2 x 2 bedroom @ $380/wk each" -> 2 x $380 = $760/wk
...
_RE_UNITS = re.compile(
    r'(\d{1,2})\s*[xX\xd7]\s*(\d)\s*(?:bed(?:room)?s?|br)'
...
# Sanity cap: suburb median weekly rent for residential property.
# Values above this indicate corrupt/misaligned data in the stats spreadsheet
# (e.g. a median sale price landing in the rent_house_pw column — seen for Clinton SA).
_MAX_SANE_RENT_PW = 2_000
...
def estimate_rent(desc, beds='', ptype='', suburb='', state=''):
    """
    ...
    if unit_items:
        ...
        return {'annual_rent': int(total_pw * 52), 'weekly_rent': int(total_pw),
                'assumptions': parts_str, 'rent_source': 'description'}

    # 2. Individual units listed separately: "Unit 1: $350pw, Unit 2: $380pw"
    ...
    if pa:
        ...
        return {'annual_rent': annual, 'weekly_rent': int(annual / 52),
                'assumptions': f"{label}${annual:,}/yr (desc)", 'rent_source': 'description'}

    # 4. Weekly amounts from description
    pw = _pw_amounts(text)
    if pw:
    ...
    # Guard: values above _MAX_SANE_RENT_PW indicate a corrupt/misaligned row
    # in the stats spreadsheet (e.g. Clinton SA has rent_house_pw = 297500).
    if suburb:
        ...
        if med_pw and med_pw <= _MAX_SANE_RENT_PW:
            kind = 'House' if is_house else 'Unit'
            return {'annual_rent': int(med_pw * 52), 'weekly_rent': int(med_pw),
